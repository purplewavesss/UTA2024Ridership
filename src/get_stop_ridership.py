...
def main():
    ...
    agg.write_to_file(station_ridership, f"bus_stop_ridership_{int(cmd_args[2])}.csv", "Stop")
...
def calculate_data(cmd_args: list[str], data: pd.DataFrame) -> dict[str, int]:
    ...
    for stop in stops:
        stop_ridership.update({stop: agg.get_ridership(year, stop, data, "stopname", 365, True)})
